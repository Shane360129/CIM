...
| 🎮 **聊天室小遊戲** | 圈圈叉叉、五子棋（對戰：兩個座位輪流下）、2048（合作：輪流接手，同一時間只有一位在操作），棋盤存在伺服器，聊天室成員看到的都是同一盤 |
...
| 🎮 **小遊戲** | 選一款就在聊天室開一局，訊息列上直接看得到盤面，成員都能加入。玩到一半想換款式，遊戲視窗下方按「換個遊戲」就好。 |
| ✅ **待辦清單** | 全家共用，誰加的、誰完成的都看得到。 |
| 📅 **行事曆** | 生日、聚餐、繳費提醒，到時間全家都會收到通知。 |

**小遊戲怎麼玩**

- **圈圈叉叉、五子棋（兩人對戰）**：開局的人自動坐第一個位子，另一位按「加入對戰」坐第二個位子，
  之後輪流下，沒輪到你的時候格子點不動；其他成員可以在旁邊看。分出勝負後按「再來一局」會換先手，
  戰績（幾勝幾負）會一直累積。五子棋是 13 路棋盤，格子小，按「放大遊玩」開棋盤才能下。
- **2048（一起玩）**：大家共用同一盤，但**同一時間只有一位「操作者」**能動 —— 按「我要玩」接手，
  換人時按「換人玩」放開，別人才接得到（操作者閒置超過 1 分鐘，其他人可以直接接手，避免卡住）。
  可以用畫面上的方向鍵、鍵盤方向鍵或直接在盤面上滑動；分數與最佳分數大家一起累積。
...
- **小遊戲的盤面與規則都在伺服器端**（`games` 資料表，一則遊戲訊息一列）：
  誰能下、輪到誰、有沒有連成線、2048 要在哪裡長出新數字，全部由 Durable Object 判定後廣播，
  前端只負責畫。所以兩邊看到的一定是同一盤，也沒辦法從瀏覽器改盤面作弊。
...
| 跑小遊戲規則測試 | `npm run test:games`（2048 合併與死局、圈圈叉叉／五子棋連線判定；不需要啟動伺服器） |
...
scripts/test-games.mjs 小遊戲規則單元測試（2048、連線判定）
